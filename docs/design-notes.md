...
이 문서는 MeshBoard 가 왜 지금의 구조를 갖게 됐는지, 각 판단의 근거가 무엇인지를 정리합니다.
기능 목록과 실행 방법은 [README](../README.md) 를 참고하세요.

## 경계 나누기

에이전트가 늘어날 때 생기는 네 가지 질문 — 누가 받는가, 누가 강제하는가, 어떻게 추적하는가,
어떻게 안전하게 검증하는가 — 을 각각 담당하는 경계로 나눴습니다.

| 경계 | 책임 |
|---|---|
| Registry | 에이전트 메타데이터, 도구 allow-list, 구독 규칙 |
| Workspace | 배치, 멤버십, 그래프 토폴로지, Goal |
| Message Broker | 타겟 해소와 fan-out, receipt, 실행 트리 기록 |
| Trust | 정책 템플릿 검증과 실행 전 강제, 인증 |
| Operations | 실행 트리 조회, 모델·병렬 분석, 아카이브, 운영자 제어 |
| Sandbox | 운영 데이터를 건드리지 않는 라우팅 시뮬레이션 |
...
1. `SEED_DB=1 RUN_APP=1 ./init.sh` 로 데모 데이터를 준비하고 앱을 띄웁니다.
...
다음은 의도적으로 구현하지 않았습니다. 각 항목은 지금 구조에서 확장 가능한 지점이기도 합니다.
...
이 경계는 README 와 API 응답(`graph.durable=false`)에 그대로 드러냅니다.
